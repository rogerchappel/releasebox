...
test('readiness check reports missing releaseforge files', async () => {
  const root = await mkdtemp(join(tmpdir(), 'releaseforge-check-'));
  ...
  assert.equal(results.some((result) => result.name === 'releaseforge config' && !result.ok), true);
});
...
test('readiness check accepts basic node cli package metadata', async () => {
  ...
  await writeFile(join(root, 'releaseforge.config.json'), JSON.stringify({ projectType: 'node-cli' }));
