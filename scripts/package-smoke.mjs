...
const tmp = await mkdtemp(join(tmpdir(), 'releaseforge-package-smoke-'));
...
try {
  ...
  execFileSync('npx', ['releaseforge', '--help'], { cwd: tmp, stdio: 'inherit' });
  execFileSync('npx', ['releaseforge', '--version'], { cwd: tmp, stdio: 'inherit' });
